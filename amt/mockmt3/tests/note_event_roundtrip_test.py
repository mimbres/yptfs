...
from mockmt3.utils.note_event_dataclasses import Note, NoteEvent, Event
from mockmt3.utils.note2event import note2note_event, note_event2event
from mockmt3.utils.note2event import validate_notes, trim_overlapping_notes
from mockmt3.utils.event2note import event2note_event, note_event2note
from mockmt3.utils.tokenizer import EventTokenizer, NoteEventTokenizer
from mockmt3.utils.midi import note_event2midi
from mockmt3.utils.midi import midi2note
...
from mockmt3.config.vocabulary import GM_INSTR_FULL, SINGING_SOLO_CLASS
# yapf: disable
...
class TestNoteEventRoundTrip1(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.note_events = [
            ...
            NoteEvent(is_drum=True, program=128, time=0.2, velocity=100, pitch=36, activity=set()),
            ...
            NoteEvent(is_drum=False, program=100, time=1.6, velocity=100, pitch=77, activity=set()),
            NoteEvent(is_drum=False, program=100, time=2.0, velocity=0, pitch=77, activity=set()),
            NoteEvent(is_drum=True, program=128, time=2.0, velocity=100, pitch=38, activity=set()),
...
class TestNoteEvent2(unittest.TestCase):
    ...
    def setUp(self) -> None:
        notes = [
            ...
            Note(is_drum=False, program=25, onset=0.4, offset=1.1, pitch=55, velocity=100),
            Note(is_drum=True, program=128, onset=1, offset=1.01, pitch=42, velocity=100),
            Note(is_drum=False, program=33, onset=1.2, offset=1.8, pitch=80, velocity=100),
            Note(is_drum=False, program=33, onset=1.6, offset=2.0, pitch=62, velocity=100),
            Note(is_drum=False, program=100, onset=1.6, offset=2.0, pitch=77, velocity=100),
            Note(is_drum=False, program=98, onset=1.7, offset=2.0, pitch=77, velocity=100),
            Note(is_drum=True, program=128, onset=1.9, offset=1.91, pitch=38, velocity=100)
            ]
