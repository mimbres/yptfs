...
from mockmt3.utils.note2event import note2note_event
from mockmt3.utils.note2event import slice_note_events_and_ties
from mockmt3.utils.note2event import slice_multiple_note_events_and_ties_to_bundle
from mockmt3.utils.note2event import trim_overlapping_notes
from mockmt3.utils.note2event import note_event2event
from mockmt3.utils.note2event import mix_notes
from mockmt3.utils.note2event import validate_notes
from mockmt3.utils.note_event_dataclasses import Note, NoteEvent, NoteEventListsBundle
from mockmt3.utils.note_event_dataclasses import Event
...
class TestNoteTools(unittest.TestCase):
    ...
    def test_mix_notes(self):
        notes1 = [
            Note(is_drum=False, program=33, onset=0.0, offset=0.5, pitch=60, velocity=100),
            Note(is_drum=False, program=33, onset=1.0, offset=1.5, pitch=62, velocity=100),
            Note(is_drum=True, program=128, onset=2.0, offset=2.1, pitch=36, velocity=100)
        ]
        notes2 = [
            Note(is_drum=False, program=52, onset=0.5, offset=1.0, pitch=40, velocity=100),
            Note(is_drum=False, program=100, onset=1.5, offset=2.0, pitch=77, velocity=100),
            Note(is_drum=True, program=128, onset=2.5, offset=2.6, pitch=38, velocity=100)
        ]
    ...
    def test_validate_notes(self):
        ...
        notes = [
            Note(is_drum=False, program=33, onset=0.0, offset=0.5, pitch=60, velocity=100),
            Note(is_drum=False, program=33, onset=1.0, offset=0.9, pitch=62, velocity=100),
            Note(is_drum=True, program=128, onset=2.0, offset=2.1, pitch=36, velocity=100),
            Note(is_drum=False, program=100, onset=1.5, offset=1.4, pitch=77, velocity=100)
        ]
        ...
        expected_validated_notes = [
            Note(is_drum=False, program=33, onset=0.0, offset=0.5, pitch=60, velocity=100),
            Note(is_drum=False, program=33, onset=1.0, offset=1.0 + MINIMUM_OFFSET_TIME, pitch=62, velocity=100),
            Note(is_drum=True, program=128, onset=2.0, offset=2.1, pitch=36, velocity=100),
            Note(is_drum=False, program=100, onset=1.5, offset=1.5 + MINIMUM_OFFSET_TIME, pitch=77, velocity=100)
        ]
...
class TestNoteEvent(unittest.TestCase):
    ...
    def test_NoteEvent(self):
        note_event = NoteEvent(is_drum=False, program=33, time=0, velocity=100, pitch=60)
...
class TestNote2NoteEvent(unittest.TestCase):
    def test_note2note_event(self):
        notes = [
            Note(is_drum=False, program=33, onset=0, offset=1.5, pitch=60, velocity=100),
            Note(is_drum=False, program=33, onset=1.6, offset=3.0, pitch=62, velocity=100),
            Note(is_drum=False, program=100, onset=1.6, offset=2.0, pitch=77, velocity=100),
            Note(is_drum=True, program=128, onset=0.2, offset=0.21, pitch=36, velocity=100),
            Note(is_drum=True, program=128, onset=2.5, offset=2.51, pitch=38, velocity=100)
            ]

        note_events = note2note_event(notes, sort=False, return_activity=False)
        self.assertSequenceEqual(note_events,
        [NoteEvent(is_drum=False, program=33, time=0, velocity=100, pitch=60),
        NoteEvent(is_drum=False, program=33, time=1.5, velocity=0, pitch=60),
        NoteEvent(is_drum=False, program=33, time=1.6, velocity=100, pitch=62),
        NoteEvent(is_drum=False, program=33, time=3.0, velocity=0, pitch=62),
        NoteEvent(is_drum=False, program=100, time=1.6, velocity=100, pitch=77),
        NoteEvent(is_drum=False, program=100, time=2.0, velocity=0, pitch=77),
        NoteEvent(is_drum=True, program=128, time=0.2, velocity=100, pitch=36),
        NoteEvent(is_drum=True, program=128, time=2.5, velocity=100, pitch=38)
        ])

        note_events = note2note_event(notes, sort=True, return_activity=True)
        self.assertSequenceEqual(note_events,
        [NoteEvent(is_drum=False, program=33, time=0, velocity=100, pitch=60, activity=set()),
        NoteEvent(is_drum=True, program=128, time=0.2, velocity=100, pitch=36, activity={0}),
        NoteEvent(is_drum=False, program=33, time=1.5, velocity=0, pitch=60, activity={0}),
        NoteEvent(is_drum=False, program=33, time=1.6, velocity=100, pitch=62, activity=set()),
        NoteEvent(is_drum=False, program=100, time=1.6, velocity=100, pitch=77, activity={3}),
        NoteEvent(is_drum=False, program=100, time=2.0, velocity=0, pitch=77, activity={3, 4}),
        NoteEvent(is_drum=True, program=128, time=2.5, velocity=100, pitch=38, activity={3}),
    ...
    def test_note2note_event_invalid_velocity_value(self):
        notes = [Note(is_drum=0, program=1, onset=0, offset=127, pitch=60, velocity=128)]
        with self.assertRaises(ValueError):
            note2note_event(notes)

    def test_note2note_event_non_empty_notes_list(self):
        notes = [
            Note(is_drum=0, program=1, onset=0, offset=127, pitch=60, velocity=100),
            Note(is_drum=0, program=1, onset=20, offset=127, pitch=62, velocity=100),
            Note(is_drum=0, program=1, onset=40, offset=127, pitch=64, velocity=100)
        ]
    ...
    def test_note2note_event_sort_parameter(self):
        notes = [
            Note(is_drum=0, program=10, onset=0, offset=127, pitch=64, velocity=100),
            Note(is_drum=0, program=10, onset=20, offset=127, pitch=60, velocity=100),
            Note(is_drum=0, program=10, onset=0, offset=127, pitch=62, velocity=100)
        ]
...
class TestNoteEventTools(unittest.TestCase):
    ...
    def test_slice_note_events_and_ties_tidyup(self):
        ...
        # Check if the tie_note_events are as expected
        expected_tie_note_events = [
            NoteEvent(is_drum=False, program=33, time=None, velocity=100, pitch=60, activity=None),
        ]
        self.assertSequenceEqual(tie_note_events, expected_tie_note_events)
        ...
        expected_sliced_note_events = [
            ...
            NoteEvent(is_drum=False, program=33, time=1.6, velocity=100, pitch=62, activity=None),
            NoteEvent(is_drum=False, program=100, time=1.6, velocity=100, pitch=77, activity=None),
            NoteEvent(is_drum=False, program=100, time=2.0, velocity=0, pitch=77, activity=None),
            NoteEvent(is_drum=True, program=128, time=2.5, velocity=100, pitch=38, activity=None)
        ]
        self.assertSequenceEqual(sliced_note_events, expected_sliced_note_events)
    ...
    def test_slice_note_events_and_ties_index_out_of_range(self):
        note_events = [
            NoteEvent(is_drum=False, program=33, time=0.1, velocity=100, pitch=60, activity=set()),
            NoteEvent(is_drum=True, program=128, time=0.2, velocity=100, pitch=36, activity={0}),
            NoteEvent(is_drum=False, program=33, time=1.5, velocity=0, pitch=60, activity={0}),
            NoteEvent(is_drum=True, program=128, time=6, velocity=100, pitch=36, activity=set())
            ]
...
class TestNoteEventToolsMultiSlice(unittest.TestCase):
    ...
    def setUp(self):
        self.note_events = [
            ...
            NoteEvent(is_drum=False, program=50, time=5.5, velocity=100, pitch=55, activity=set()),
            NoteEvent(is_drum=False, program=33, time=6.1, velocity=100, pitch=64, activity={9}),
            NoteEvent(is_drum=True, program=128, time=6.5, velocity=100, pitch=36, activity={9, 10}),
    ...
    def test_slice_multiple_note_events_and_ties_to_bundle(self):
        ...
        expected_tne_1 = [NoteEvent(is_drum=False, program=33, time=0, velocity=100, pitch=60, activity=set())]
...
class TestNoteEvent2Event(unittest.TestCase):
    ...
    def test_multiple_drum_event(self):
        note_events = [NoteEvent(is_drum=True, program=128, time=0.105, velocity=100, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=0.11499999999999999, velocity=0, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=1.5, velocity=100, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=1.51, velocity=0, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=2.886, velocity=100, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=2.896, velocity=0, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=5.528, velocity=100, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=5.537999999999999, velocity=0, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=7.641, velocity=100, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=7.651, velocity=0, pitch=38, activity=set()),
            NoteEvent(is_drum=True, program=128, time=10.413, velocity=100, pitch=38, activity=set()),
