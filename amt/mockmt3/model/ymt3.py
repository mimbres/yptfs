...
from mockmt3.model.t5mod import T5EncoderYMT3, T5DecoderYMT3, MultiChannelT5Decoder
from mockmt3.model.t5mod_helper import task_cond_dec_generate
from mockmt3.model.perceiver_mod import PerceiverTFEncoder
from mockmt3.model.perceiver_helper import PerceiverTFConfig
from mockmt3.model.conformer_mod import ConformerYMT3Encoder
from mockmt3.model.conformer_helper import ConformerYMT3Config
from mockmt3.model.lm_head import LMHead
from mockmt3.model.pitchshift_layer import PitchShiftLayer
from mockmt3.model.spectrogram import get_spectrogram_layer_from_audio_cfg
from mockmt3.model.conv_block import PreEncoderBlockRes3B
from mockmt3.model.conv_block import PreEncoderBlockHFTT, PreEncoderBlockRes3BHFTT  # added for hFTT-like pre-encoder
from mockmt3.model.projection_layer import get_projection_layer, get_multi_channel_projection_layer
from mockmt3.model.optimizers import get_optimizer
from mockmt3.model.lr_scheduler import get_lr_scheduler

from mockmt3.utils.note_event_dataclasses import Note
from mockmt3.utils.note2event import mix_notes
from mockmt3.utils.event2note import merge_zipped_note_events_and_ties_to_notes, DECODING_ERR_TYPES
from mockmt3.utils.metrics import compute_track_metrics
from mockmt3.utils.metrics import AMTMetrics
# from mockmt3.utils.utils import write_model_output_as_npy
from mockmt3.utils.utils import write_model_output_as_midi, create_inverse_vocab, write_err_cnt_as_json
from mockmt3.utils.utils import Timer
from mockmt3.utils.task_manager import TaskManager

from mockmt3.config.config import audio_cfg as default_audio_cfg
from mockmt3.config.config import model_cfg as default_model_cfg
from mockmt3.config.config import shared_cfg as default_shared_cfg
from mockmt3.config.config import T5_BASE_CFG
...
class YourMT3(pl.LightningModule):
    ...
    def __init__(
        ...
        model_cfg["feat_length"] = spec_output_shape[0]
        ...
        assert model_cfg["encoder_type"] in ["t5", "perceiver-tf", "conformer", None]
        ...
        if model_cfg["pre_decoder_type"] == "default":
            if model_cfg["encoder_type"] is not None:
                model_cfg["pre_decoder_type"] = model_cfg["pre_decoder_type_default"].get(
                    model_cfg["encoder_type"]).get(model_cfg["decoder_type"], None)
        ...
        elif self.pre_encoder_type == "m2l-linear":
            self.pre_encoder.append(nn.Linear(64, 512, bias=False))
            pre_enc_output_shape = spec_output_shape
        elif self.pre_encoder_type == "m2l-fe-linear":
            self.pre_encoder.append(nn.Linear(8192, 512, bias=False))
            pre_enc_output_shape = spec_output_shape
        else:
        ...
        elif self.encoder_type in ["t5", "conformer", None] and self.decoder_type == "t5":
            pass
        ...
        if self.encoder_type in ["t5", "conformer", None]:
    ...
    def set_encoder_decoder(self) -> None:
        ...
        if self.encoder_type == "t5":
            ...
            class EmptyEnc(nn.Module):

                def __init__(self):
                    super(EmptyEnc, self).__init__()
                    self.enc = nn.Identity()

                def forward(self, inputs_embeds: torch.Tensor):
                    return {"last_hidden_state": self.enc(inputs_embeds)}

            self.encoder = EmptyEnc()
    ...
    def setup(self, stage: str) -> None:
        ...
        # Add velocity metric
        self.add_velocity_metric = self.task_manager.base_codec == 'mt3_vel'
    ...
    def inference_file(
        ...
        """ Inference from audio batch (from audio file of variable length) by autoregressive decoding:
        Args:
            bsz: batch size
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0) -> Dict:
        ...
        with Timer() as t:
            ...
            if self.hparams.write_output_dir is not None:
                ...
                write_model_output_as_midi(pred_notes,
                                           ...
                                           output_dir_suffix=f"{dataset_info}_{self.hparams.eval_subtask_key}",
                                           input_binary_velocity=(self.task_manager.base_codec == 'mt3'))
            ...
            drum_metric, non_drum_metric, instr_metric = compute_track_metrics(
                ...
                add_velocity_metric=self.add_velocity_metric)
        ...
        #     actual_events_text = [str(self.task_manager.tokenizer._decode(actual_tokens))]
        ...
        self.log('val_loss', loss, prog_bar=True, batch_size=n_items, sync_dist=False)  #True)
        # self.val_metrics[dataloader_idx].bulk_update_errors({'decoding_time': decoding_time_sec})

    @rank_zero_only
    def on_validation_epoch_end(self) -> None:
        # rank_zero_only and sync_dist for H100-nvl
        for val_metrics in self.val_metrics:
            self.log_dict(val_metrics.bulk_compute(), rank_zero_only=True)  # ,sync_dist=True)
            val_metrics.bulk_reset()
        self.log_dict(self.val_metrics_macro.bulk_compute(), rank_zero_only=True)  # ,sync_dist=True)
        ...
        if len(pred_token_array_file) > 0:
            ...
            if self.hparams.write_output_dir is not None and self.global_rank == 0:
                ...
                write_model_output_as_midi(
                    ...
                    track_info,
                    self.midi_output_inverse_vocab,
                    output_dir_suffix=f"{dataset_info}_{self.hparams.eval_subtask_key}_{f_score}",
                    input_binary_velocity=(self.task_manager.base_codec == 'mt3'))
                write_err_cnt_as_json(track_info, self.hparams.write_output_dir,
                                      f"{dataset_info}_{self.hparams.eval_subtask_key}_{f_score}", n_err_cnt,
                                      ne_err_cnt)
...
def test_case_forward_mt3():
    import torch
    from mockmt3.config.config import audio_cfg, model_cfg, shared_cfg
    from mockmt3.model.ymt3 import YourMT3
    model = YourMT3()
    model.eval()
