...
from mockmt3.config.config import DEEPSPEED_CFG

# Manage warnings
disable_possible_user_warnings()
warnings.filterwarnings("ignore", ".*It is recommended to use*")
warnings.filterwarnings("ignore", ".*find_unused_parameters*")


def initialize_trainer(args: argparse.Namespace,
                       stage: Literal['train', 'test'] = 'train',
                       log_dir: str = None) -> Tuple[pl.Trainer, WandbLogger, dict]:
    ...
    if log_dir is None:
        ...
        lightning_dir = os.path.join(log_dir, args.project, args.exp_id)
    ...
    wandb_logger = WandbLogger(
        log_model=True,  # "all": save all the checkpoints to wandb artifacts, True: only the end of training 
    ...
    last_ckpt_path = os.path.join(checkpoint_dir, checkpoint_name)
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=checkpoint_dir, **shared_cfg["CHECKPOINT"])
    ...
    # limit logging frequency to avoid system OOM
    if stage == 'train':
        if args.val_interval is not None:
            if torch.cuda.is_available():
                num_gpus = torch.cuda.device_count()
                log_every_n_steps = 100
                # log_every_n_steps = max(int(args.val_interval / 200 * num_gpus), 50)
            else:
                log_every_n_steps = max(int(args.val_interval / 200), 50)
        else:
            log_every_n_steps = 50
    else:
        log_every_n_steps = None
    print(f"🧑‍💻Logging loss every {log_every_n_steps} steps...")
...
def update_config(args, shared_cfg, stage: Literal['train', 'test'] = 'train'):
    ...
    if args.audio_codec != None:
        assert args.audio_codec in ['spec', 'melspec', 'm2l', 'm2l-fe', 'm2l-fe-1x1']
    ...
    if args.encoder_type != None:
        if args.encoder_type in ["None", "none", "0", 0]:
