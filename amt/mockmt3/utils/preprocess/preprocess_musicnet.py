...
import numpy as np
from mockmt3.utils.audio import get_audio_file_info
...
# yapf: enable
MUSICNET_DISCARD_INFO = ['test_labels_midi/1759.mid', 'test_labels_midi/1819.mid']  # duplicated midi files
...
def create_note_event_and_note_from_midi(mid_file: str, id: str) -> Tuple[Dict, Dict]:
    ...
    Returns:
        ...
        note_events (dict): note events and metadata.
    """
    notes, dur_sec = midi2note(mid_file,
...
def preprocess_musicnet16k(data_home=os.PathLike, dataset_name='musicnet') -> None:
    ...
    mid_em_pattern = os.path.join(base_dir, '*_em', '*.mid')  # EM annotations for real performances (wav)
    ...
    for file in MUSICNET_DISCARD_INFO:
        if os.path.join(base_dir, file) in mid_files:
            mid_files.remove(os.path.join(base_dir, file))
    print(len(mid_files), 'mid_files')
    print(len(label_files), 'label_files')
    print(len(wav_synth_files), 'wav_synth_files')
    print(len(wav_acoustic_files), 'wav_acoustic_files')
    assert (len(mid_files) == len(label_files) == len(wav_synth_files) == len(wav_acoustic_files) == 330)
    ...
    def search_file_by_musicnet_id(musicnet_id, files):
        file_found = [f for f in files if musicnet_id in f]  # this only works in 4-digits file names of MusicNet
    ...
    # Process label files
    for i in musicnet_ids:
        notes, note_events = create_note_event_and_note_from_label(label_file=musicnet_dict[i]['label_file'], id=i)
    ...
    for i in musicnet_ids:
        # musicnet
        notes, note_events = create_note_event_and_note_from_midi(mid_file=musicnet_dict[i]['mid_file'], id=i)
        ...
        # update musicnet_dict
        musicnet_dict[i]['duration_sec'] = max(notes['duration_sec'], musicnet_dict[i]['duration_sec'])
        ...
        if i in musicnet_em_ids:
            notes, note_events = create_note_event_and_note_from_midi(mid_file=musicnet_dict[i]['mid_em_file'], id=i)
            notes['program'] = musicnet_dict[i]['program'].copy()
            notes['is_drum'] = musicnet_dict[i]['is_drum'].copy()
            ...
            note_events['program'] = musicnet_dict[i]['program'].copy()
            note_events['is_drum'] = musicnet_dict[i]['is_drum'].copy()
            note_events_file = os.path.join(musicnet_dict[i]['mid_em_file'][:-4] + '_note_events.npy')
            np.save(note_events_file, note_events, allow_pickle=True, fix_imports=False)
            print(f'Created {note_events_file}')
    ...
    split_dict['train_thickstun_em'] = whole_set - set(split_dict['test_thickstun_ext']) - MUSICNET_EM_MISSING_IDS
    ...
    for split in ('train_thickstun_em', 'train_mt3_em'):
        ...
        synth_ids = split_dict['train_mt3'] if split == 'train_mt3_em' else split_dict['train_thickstun']
    ...
    for split in ('validation_mt3_em', 'test_mt3_em', 'test_em_table2', 'test_thickstun_em', 'test_thickstun_ext_em'):
        file_list = {}
        for i, musicnet_id in enumerate(split_dict[split]):
            file_list[i] = {
                'musicnet_id': musicnet_id,
