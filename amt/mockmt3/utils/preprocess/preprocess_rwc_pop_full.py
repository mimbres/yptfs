...
from mockmt3.utils.note2event import note2note_event, sort_notes, validate_notes, trim_overlapping_notes, extract_program_from_notes
from mockmt3.utils.event2note import event2note_event
from mockmt3.utils.note_event_dataclasses import Note, NoteEvent
from mockmt3.utils.utils import note_event2token2note_event_sanity_check
...
def generate_corrected_midi(org_mid_file: os.PathLike, new_mid_file: os.PathLike, filters: Dict[Any, Dict[str, List]],
    ...
    for track in org_mid.tracks[1:]:
        ...
        if program == DRUM_PROGRAM:
            ...
            new_track.append(Message('program_change', program=program, time=0, channel=ch))
...
def preprocess_rwc_pop_full16k(data_home='../../data', dataset_name='rwc_pop') -> None:
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            id = row[2]
            mix_audio_file = os.path.join(base_dir, headers[0] + row[0], row[1] + ' ' + headers[1] + '.wav')
    ...
    for id, info in rwc_all.items():
        ...
        generate_corrected_midi(org_mid_file, new_mid_file, filters=TRACK_NAME_FILTERS, prg2ch=PRG2CH)
        ...
        rwc_all[id]['note_events_file'] = new_mid_file.replace('.mid', '_note_events.npy')
