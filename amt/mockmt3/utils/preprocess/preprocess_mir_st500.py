...
def create_note_note_event_midi_from_mir_st500_annotation(ann, midi_file, mir_st500_id):
    ...
    for onset, offset, pitch in ann:
        notes.append(
            Note(is_drum=False, program=100, onset=float(onset), offset=float(offset), pitch=int(pitch), velocity=100))
...
def preprocess_mir_st500_16k(data_home=os.PathLike,
    ...
    for i in range(1, 501):
        ...
        if check_file_existence(audio_file) and check_file_existence(audio_vocal_file) and check_file_existence(
                audio_acc_file):
            audio_all[str(i)] = audio_file
        else:
            audio_missing[split].append(i)
    print(f'Number of missing audio files: train = {len(audio_missing["train"])}, test = {len(audio_missing["test"])}')
    ...
    for id in ids_all:
        ...
        notes, note_events = create_note_note_event_midi_from_mir_st500_annotation(ann, midi_file, id)
    ...
    for split in ['train', 'train_vocal', 'train_stem', 'test', 'test_vocal', 'all', 'all_vocal', 'all_stem']:
        file_list = {}
