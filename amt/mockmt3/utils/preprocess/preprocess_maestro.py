...
def preprocess_maestro16k(data_home=os.PathLike,
    ...
    for i in ids_all:
        ...
        # Rename '.midi' to '.mid'
        if '.midi' in m['midi_filename'] and not os.path.exists(m['midi_filename'].replace('.midi', '.mid')):
    ...
    for i in ids['all']:
        ...
        # Process MIDI
        notes, note_events = create_note_event_and_note_from_midi(mid_file=midi_file, id=i, ignore_pedal=ignore_pedal)
