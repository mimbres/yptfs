...
from mockmt3.utils.audio import get_audio_file_info, pitch_shift_audio
from mockmt3.utils.midi import note_event2midi, pitch_shift_midi
...
def preprocess_guitarset16k(data_home: os.PathLike,
    ...
    # Create filelist for progression [1, 2] w/ pitch shift [-2, 0, 2]
    create_filelist_progression_pshift_pm2_guitarset16k(data_home, dataset_name)
...
def create_filelist_progression_pshift_pm2_guitarset16k(data_home: os.PathLike,
                                                        dataset_name: str = 'guitarset') -> None:
    pitch_shift_values = [-2, 0, 2]
    progressions_for_train = [1, 2]
    ...
    for n in progressions_for_train:
        # create a new file_list for progression n
        fl_pm2 = {}
        idx = 0

        # Load filelist for progression n with pitch shift(-5,-4,..0,..4,5)
        file_list_progression_n_pshift_all_path = os.path.join(output_index_dir,
                                                               f'{dataset_name}_progression_{n}_pshift_file_list.json')

        with open(file_list_progression_n_pshift_all_path, 'r') as f:
            fl_pshift = json.load(f)
        assert len(fl_pshift) == 2640

        for v in fl_pshift.values():
            if v.get('pitch_shift', 0) in pitch_shift_values:
                fl_pm2[str(idx)] = copy.deepcopy(v)
                idx += 1

        assert len(fl_pm2) == 720

        # write file_list
        output_file = os.path.join(output_index_dir, f'{dataset_name}_progression_{n}_pshift_pm2_file_list.json')
        with open(output_file, 'w') as f:
            json.dump(fl_pm2, f, indent=4)
        print(f'Created {output_file}')
