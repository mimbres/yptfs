...
from mockmt3.utils.audio import get_audio_file_info, load_audio_file, write_wav_file
from mockmt3.utils.midi import note_event2midi
from mockmt3.utils.note2event import note2note_event, sort_notes, validate_notes, trim_overlapping_notes, mix_notes
from mockmt3.config.vocabulary import ENST_DRUM_NOTES
...
def preprocess_enstdrums16k(data_home: os.PathLike, dataset_name='enstdrums') -> None:
    ...
    enst_info = {}
    for i in [1, 2, 3]:
        drummer_files = sorted(glob.glob(os.path.join(base_dir, f'drummer_{i}', 'annotation/*.txt')))
        for file in drummer_files:
            ...
            if 'minus-one' in file:  # unannotated accompaniment exists
                ...
                os.makedirs(os.path.join(base_dir, f'drummer_{i}', 'audio', 'accompaniment_mix_r1'), exist_ok=True)
                accomp_mix_audio_file_r1 = os.path.join(base_dir, f'drummer_{i}', 'audio', 'accompaniment_mix_r1',
                                                        os.path.basename(file).replace('.txt', '.wav'))
                accomp_mix_audio_r1 = stem_content['audio_array'][0] / np.max(np.abs(
                    stem_content['audio_array'][0])) * 0.75 + stem_content['audio_array'][1] / np.max(
                        np.abs(stem_content['audio_array'][1])) * 0.25
                ...
                # create (drum + accompaniment) mix audio file. r1
                os.makedirs(os.path.join(base_dir, f'drummer_{i}', 'audio', 'accompaniment_mix_r2'), exist_ok=True)
                accomp_mix_audio_file_r2 = os.path.join(base_dir, f'drummer_{i}', 'audio', 'accompaniment_mix_r2',
                                                        os.path.basename(file).replace('.txt', '.wav'))
                accomp_mix_audio_r2 = stem_content['audio_array'][0] / np.max(np.abs(
                    stem_content['audio_array'][0])) * 0.6 + stem_content['audio_array'][1] / np.max(
                        np.abs(stem_content['audio_array'][1])) * 0.4
    ...
    # Write index
    for split in [
            'drummer_1_dtm', 'drummer_2_dtm', 'all_dtm', 'drummer_1_dtp', 'drummer_2_dtp', 'all_dtp', 'drummer_3_dtd',
            'drummer_3_dtp', 'drummer_3_dtm_r1', 'drummer_3_dtm_r2'
    ]:
        # splits for training
        file_list = {}
        i = 0
...
def create_filelist_dtm_random_enstdrums16k(data_home: os.PathLike, dataset_name: str = 'enstdrums') -> None:
    ...
    # Load all filelist
    file_list_all_dtm_path = os.path.join(output_index_dir, f'{dataset_name}_all_dtm_file_list.json')
    file_list_all_dtp_path = os.path.join(output_index_dir, f'{dataset_name}_all_dtp_file_list.json')
    ...
    idx = {}
    idx['train_dtm'] = [
        47, 58, 14, 48, 60, 44, 34, 31, 5, 62, 46, 12, 9, 26, 57, 11, 16, 22, 33, 3, 6, 55, 50, 32, 52, 53, 10, 28, 24,
        41, 63, 51, 43, 49, 54, 15, 20, 1, 27, 2, 23, 45, 38, 37
    ]
        ...
        if 'dtm' in split:
            ...
            if split == 'test_dtm' or split == 'validation_dtm':
                ...
                for k, v in fl_dtm.items():
                    if int(k) in idx[split]:
                        _v = copy.deepcopy(v)
                        _v['mix_audio_file'] = _v['mix_audio_file'].replace('accompaniment_mix_r2',
            ...
            for k, v in fl_dtm.items():
                if int(k) in idx[split]:
                    _v = copy.deepcopy(v)
                    _v['stem_file'] = None
                    _v['mix_audio_file'] = _v['mix_audio_file'].replace('accompaniment_mix_r2', 'wet_mix')
