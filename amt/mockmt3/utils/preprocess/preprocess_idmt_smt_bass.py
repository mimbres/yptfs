...
from mockmt3.utils.audio import get_audio_file_info, load_audio_file, write_wav_file, guess_onset_offset_by_amp_envelope
...
def regenerate_stratified_split(audio_files_dict):
    ...
    for key, files in audio_files_dict.items():
        ...
        train_ids, val_ids = train_test_split(ids, test_size=0.2, random_state=42, stratify=np.zeros_like(ids))
...
def preprocess_idmt_smt_bass_16k(data_home=os.PathLike,
    ...
    for id in all_info_dict.keys():
        ...
        _, _, _, _, pluck_style, _, string_num, fret_num = os.path.basename(audio_file).split('.')[0].split('_')
        ...
        onset, offset, _ = guess_onset_offset_by_amp_envelope(x,
                                                              fs=fs,
        ...
        notes_dict, note_events_dict = create_note_event_and_note_from_midi(midi_file,
                                                                            id,
                                                                            program=program,
                                                                            ignore_pedal=True)
        if sanity_check:
