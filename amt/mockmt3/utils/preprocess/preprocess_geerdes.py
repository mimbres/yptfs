...
import numpy as np
from mockmt3.utils.audio import get_audio_file_info, load_audio_file
from mockmt3.utils.midi import midi2note, note_event2midi
from mockmt3.utils.note2event import (note2note_event, sort_notes, validate_notes, trim_overlapping_notes,
                                      extract_program_from_notes, extract_notes_selected_by_programs)
from mockmt3.utils.event2note import event2note_event
from mockmt3.utils.note_event_dataclasses import Note, NoteEvent
from mockmt3.utils.utils import note_event2token2note_event_sanity_check, create_inverse_vocab
from mockmt3.config.vocabulary import MT3_FULL_PLUS
...
def find_index_with_key(lst, key):
    ...
    indices = [index for index, value in enumerate(lst) if filtered_key in filter_string(value.lower())]
...
def create_note_event_and_note_from_midi(mid_file: str,
    ...
    # Load midi file
    notes, dur_sec, program = midi2note(mid_file,
...
def preprocess_geerdes16k(data_home=os.PathLike, dataset_name='geerdes', sanity_check=False) -> None:
    """Preprocess Geerdes dataset."""
    # Directory and file paths
    base_dir = os.path.join(data_home, dataset_name + '_yourmt3_16k')
    output_index_dir = os.path.join(data_home, 'yourmt3_indexes')
    ...
    for geerdes_id, v in tracks_all.items():
        ...
        notes, note_events = create_note_event_and_note_from_midi(mid_file=midi_file,
                                                                  id=geerdes_id,
                                                                  ch_9_as_drum=True,
        ...
        if sanity_check is True:
            err_cnt = note_event2token2note_event_sanity_check(note_events['note_events'], notes['notes'])
            if len(err_cnt) > 0:
        ...
        note_event2midi(note_events['note_events'], recon_midi_file, output_inverse_vocab=inverse_vocab)
        ...
        notes_voc['notes'] = extract_notes_selected_by_programs(notes['notes'],
                                                                [SINGING_VOICE_PROGRAM, SINGING_VOICE_CHORUS_PROGRAM])
        ...
        notes_acc['notes'] = extract_notes_selected_by_programs(
            notes['notes'],
            [p for p in notes['program'] if p not in [SINGING_VOICE_PROGRAM, SINGING_VOICE_CHORUS_PROGRAM]])
    ...
    for task_suffix in task_suffixes:
        for split in splits:
            ...
            file_list_file = os.path.join(output_index_dir, f'{dataset_name}_{split}{task_suffix}_file_list.json')
