...
# from mockmt3.utils.utils import assert_note_events_almost_equal


def create_note_event_and_note_from_midi(mid_file: str, id: str, ignore_pedal: bool = False) -> Tuple[Dict, Dict]:
...
def preprocess_maps16k(data_home=os.PathLike, dataset_name='maps', ignore_pedal=False, sanity_check=False) -> None:
    """
    Splits:
    ...
    for train_file in train_mid_files:
        if not any(songname in os.path.basename(train_file) for songname in songnames_in_test_files):
    ...
    for i, mid_file in enumerate(all_mid_files):
        ...
        notes, note_events = create_note_event_and_note_from_midi(mid_file=mid_file,
                                                                  id=maps_id,
                                                                  ignore_pedal=ignore_pedal)
