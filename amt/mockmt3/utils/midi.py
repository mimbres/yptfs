...
def midi2note(file: Union[os.PathLike, str],
    ...
    for i, msg in enumerate(midi):
        ...
        if msg.type == 'program_change' and msg.channel not in reserved_channels:
            ...
            if program_state[msg.channel] != DRUM_PROGRAM and not force_all_drum:
...
def note_event2midi(note_events: List[NoteEvent],
    ...
    args:
        ...
        binary_velocity: If True, all velocity for onset are set to 100 (useful for mt3's binary velociy outputs).

    List[NoteEvent]: 
        [NoteEvent(is_drum: bool, program: int, time: Optional[float], binary_velocity: bool,
    ...
    for program in programs:
        ...
        for ne in note_events:
            if ne.program == program:
                ...
                msg_velocity = ne.velocity if binary_velocity is False else (100 if ne.velocity > 0 else 0)
                new_msg = Message(msg_note, note=ne.pitch, velocity=msg_velocity, time=delta_tick, channel=channel)

                track.append(new_msg)

    # # Bugfix: Add end-of-track message (thanks to @nateraw for the catch)
    # tempo = 500000
    ...
    # total_ticks = int(duration_seconds * 1000000 * midi.ticks_per_beat / tempo)
    # for track in midi.tracks:
