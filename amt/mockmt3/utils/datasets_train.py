...
from mockmt3.utils.audio import get_segments_from_numpy_array, load_audio_file
from mockmt3.utils.augment import (audio_random_submix_processor, combined_survival_and_stop,
                                   cross_stem_augment_processor, intra_stem_augment_processor)
from mockmt3.utils.note2event import slice_multiple_note_events_and_ties_to_bundle, slice_note_events_and_ties
from mockmt3.utils.note2event import pitch_shift_note_events
from mockmt3.utils.note_event_dataclasses import NoteEventListsBundle
...
class CachedAudioDataset(Dataset):
    ...
    def _get_rand_segments_from_cache(
            ...
            sample_excluding_fully_unannotated: bool = False,
            force_drop_unannotated_stems: bool = False) -> Tuple[NoteEventListsBundle, np.ndarray]:
        ...
        Args:
            ...
            **arguments below are typically used for sampling external sources aka. U\X.**
            sample_excluding_fully_unannotated: If True, exclude fully unannotated files from sampling. 
              Default: False.
            force_drop_unannotated_stems: If True, drop unannotated stems from the partially annotated data.
              Default: False.
        ...
        # Determine a set of sample ids to exclude
        sample_excluding_ids = set() if sample_excluding_ids is None else set(sample_excluding_ids)
        if sample_excluding_fully_unannotated is True:
            for i, d in enumerate(cache_values):
                if d['fully_unannotated'] is True:
                    sample_excluding_ids.add(i)

        if len(sample_excluding_ids) == 0:
            ...
            sampled_ids = np.random.permutation(list(set(np.arange(self.num_files_cache)) - sample_excluding_ids))[:n]
    ...
    def _update_cache(self, index) -> None:
        ...
        if UNANNOTATED_PROGRAM in audio_data['program']:
            ...
            data['fully_unannotated'] = True if len(audio_data['program']) == 1 else False
        else:
            data['has_unannotated'] = data['fully_unannotated'] = False

        data['has_unannotated'] = True if UNANNOTATED_PROGRAM in audio_data['program'] else False
