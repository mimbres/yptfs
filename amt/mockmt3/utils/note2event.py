...
NoteEvent to Event:
• note_event2event(note_events, tie_note_events, start_time, tps, sort, binary_velocity)
    -> List[Event]
    ...
    if return_activity:
        ...
        for i, ne in enumerate(note_events):
            ...
            if ne.is_drum:
                ...
                raise ValueError(f'📕 Invalid velocity: {ne.velocity} expected (0 - 127)')
        if len(activity) > 0:
...
def note_event2event(note_events: List[NoteEvent],
                     ...
                     tps: int = 100,
                     sort: bool = True,
                     binary_velocity: bool = False) -> List[Event]:
    """ note_event2event:
    ...
    - sort (bool): If True, sort the Event instances by increasing order of
        onsets, and at the same timing, by increasing order of program and pitch.
        Default is False.
    - binary_velocity (bool): If False, allows velocity values up to 127. 
        If True, velocity will only be considered 0 or 1. Default is True.
    ...
    for ne in note_events:
        ...
        if ne.is_drum:
            if not binary_velocity:
                ...
                # Legacy behavior: velocity is either 1 (on) or 0 (off)
                if ne.velocity > 0 and (velocity_state != 1 or velocity_state is None):
                    events.append(Event(type='velocity', value=1))
                    velocity_state = 1
            if ne.velocity > 0:  # Only process drum events with positive velocity
            ...
            if not binary_velocity:
                # Allow full range of velocity (0-127)
                if ne.velocity != velocity_state or velocity_state is None:
                    events.append(Event(type='velocity', value=ne.velocity))
                    velocity_state = ne.velocity
            else:
                # Velocity 0 or 1 behavior (legacy)
                if (ne.velocity > 0 and velocity_state != 1) or velocity_state is None:
                    events.append(Event(type='velocity', value=1))
                    velocity_state = 1
