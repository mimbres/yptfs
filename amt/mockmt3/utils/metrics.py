...
from mockmt3.utils.note_event_dataclasses import NoteEvent, Note
from mockmt3.utils.note2event import sort_notes, notes2pc_notes
from mockmt3.utils.event2note import note_event2note
from sklearn.metrics import average_precision_score
from mockmt3.utils.metrics_helper import (f1_measure, round_float, mir_eval_note_f1, mir_eval_frame_f1,
                                          mir_eval_melody_metric, extract_pitches_intervals_velocities_from_notes,
...
class UpdatedMeanMetric(MeanMetric):
    ...
    def __init__(self, nan_strategy: str = 'ignore', **kwargs) -> None:
        super().__init__(nan_strategy=nan_strategy, sync_on_compute=False, **kwargs)
        # super().__init__(nan_strategy=nan_strategy, **kwargs) # sync_on_compute=False for some H100-nvl system
        self._updated = False
...
class AMTMetrics(Module):
    ...
    def __init__(self,
        ...
        # Velocity (Onset) metrics
        self.velocity_f = UpdatedMeanMetric(nan_strategy=nan_strategy)
        self.velocity_mae = UpdatedMeanMetric(nan_strategy=nan_strategy)
        self.velocity_f_drum = UpdatedMeanMetric(nan_strategy=nan_strategy)
        self.velocity_mae_drum = UpdatedMeanMetric(nan_strategy=nan_strategy)
...
def compute_track_metrics(pred_notes: List[Note],
                          ...
                          add_velocity_metric: bool = False,
    ...
    Args:
        ...
        add_micro_metric: (bool) add micro-averaged metrics
        add_multi_f_metric: (bool) add multi-f metrics
        add_velocity_metric: (bool) add velocity-related metrics
    ...
    pns_drum, pns_non_drum = extract_drum_and_non_drum_notes(pred_notes)  # pns: predicted notes, rns: reference notes
    ...
    pns_drum_pi = extract_pitches_intervals_velocities_from_notes(pns_drum, is_drum=True)
    pns_non_drum_pi = extract_pitches_intervals_velocities_from_notes(pns_non_drum)
    rns_drum_pi = extract_pitches_intervals_velocities_from_notes(rns_drum, is_drum=True)
    rns_non_drum_pi = extract_pitches_intervals_velocities_from_notes(rns_non_drum)
    ...
    drum_metric = mir_eval_note_f1(pns_drum_pi['pitches'],
                                   ...
                                   est_velocities=pns_drum_pi['velocities'] if add_velocity_metric else None,
                                   ref_velocities=rns_drum_pi['velocities'] if add_velocity_metric else None,
    ...
    non_drum_metric = mir_eval_note_f1(pns_non_drum_pi['pitches'],
                                       ...
                                       est_velocities=pns_non_drum_pi['velocities'] if add_velocity_metric else None,
                                       ref_velocities=rns_non_drum_pi['velocities'] if add_velocity_metric else None,
                                       onset_tolerance=onset_tolerance,
                                       velocity_tolerance=velocity_tolerance,
                                       is_drum=False)  # micro metric will be added in the instrument-wise PRF code
        ...
        for group_name, programs in eval_vocab.items():
            ...
            if add_pitch_class_metric is not None:
                if group_name.lower() in [g.lower() for g in add_pitch_class_metric]:
                    ...
                    pns_pc_group = extract_pitches_intervals_velocities_from_notes(notes2pc_notes(pns_group))
                    rns_pc_group = extract_pitches_intervals_velocities_from_notes(notes2pc_notes(rns_group))
            ...
            # Extract Pitches (freq) and Intervals
            pns_group = extract_pitches_intervals_velocities_from_notes(pns_group)
            rns_group = extract_pitches_intervals_velocities_from_notes(rns_group)
