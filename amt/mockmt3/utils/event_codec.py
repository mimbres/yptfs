...
from mockmt3.utils.note_event_dataclasses import Event, EventRange
...
class FastCodec:
    ...
    def encode_event(self, event: Event) -> int:
        ...
        if (event.type, event.value) not in self._encode_dict:
            if event.type == 'velocity' and 0 < event.value < 128:
                event.value = 1  # Fix 2024 Oct: converting 0-127 velocity into binary velocity of original MT3
            else:
