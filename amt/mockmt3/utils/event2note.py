...
def event2note_event(events: List[Event],
        ...
        try:
            ...
            if e.type == 'shift':
                ...
                tie_note_events.append(
                    NoteEvent(is_drum=False, program=program_state, time=None, velocity=100,
                              pitch=e.value))  # velocity of tie-note-events will be ignored.
    ...
    for e in events:
        try:
            if e.type == 'shift':
                ...
                if velocity_state is None or velocity_state == 0:
                    # The occurrence of a drum event without a preceding velocity greater than 0 (onset) is grammatically incorrect, but it can happen rarely as an error.
                    # This can be revised by adding an arbitraty velocity of 100, and the error will be reported.
                    velocity_state = 100
                    note_events.append(
                        NoteEvent(is_drum=True,
                                  program=128,
                                  time=tick_state / tps,
    ...
    if sort:
        ...
        tie_note_events.sort(key=lambda n_ev: (n_ev.is_drum, n_ev.program))
    ...
    for ne in note_events:
        try:
            if ne.time == None:
                ...
                # <---- DEBUG ---->
                if ne.velocity is None:
                    print("ERROR DETECTED!! Drum Velocity is None, during note_event2note()")
                    raise ValueError('Err/Missing vel')
                # <---- DEBUG ---->
                if ne.velocity > 0:  # == 1: fix for allowing velocity up to 127
                    notes.append(
                        Note(is_drum=True,
                             program=128,
                             onset=ne.time,
                    ...
                    continue
            elif ne.velocity > 0:  # == 1: fix for allowing velocity up to 127
