        ...
        for msg in midi:
            if 'note' in msg.type:
                if is_drum and (msg.channel != 9):
                    print('found drum track with channel != 9 in track_id: ', track_id)
                if not is_drum and (msg.channel == 9):
                    print('found non-drum track with channel == 9 in track_id: ', track_id)
                if is_drum and (msg.channel == 9):
                    cnt += 1
        if cnt > 0:
            print(f'found {cnt} notes in drum track with ch 9 in track_id: ', track_id)
    return
