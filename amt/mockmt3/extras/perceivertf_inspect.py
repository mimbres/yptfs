...
def test_case_forward_enc_perceiver_tf_dec_t5():
    ...
    plt.title('ENC_HS_PROJ: linear projection of encoder output, which is used for enc-dec cross attention')
    plt.colorbar(orientation='horizontal')
    plt.ylabel('latent k')
    plt.xlabel('d')
    plt.show()
    ...
    plt.show()
    # dec catt
    plt.imshow(np.rot90((torch.sum(dec_catt[7][0], axis=0))[:1000, :].detach().numpy()), origin='upper', aspect='auto')
    plt.colorbar()
