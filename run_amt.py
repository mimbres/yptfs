...
# yapf: disable
def load_model(device, cfg, model_name='YPTF+Single (noPS)'):
    ...
    from mockmt3.model.init_train import initialize_trainer, update_config
    # from mockmt3.utils.task_manager import TaskManager
    from mockmt3.config.vocabulary import drum_vocab_presets
    from mockmt3.utils.utils import str2bool
    from mockmt3.utils.utils import Timer
    from mockmt3.utils.audio import slice_padded_array
    from mockmt3.utils.note2event import mix_notes
    from mockmt3.utils.event2note import merge_zipped_note_events_and_ties_to_notes
    from mockmt3.utils.utils import write_model_output_as_midi, write_err_cnt_as_json
    ...
    def load_model_checkpoint(args=None):
        ...
        _, _, dir_info, shared_cfg = initialize_trainer(args, stage='test', log_dir='amt/logs')
        ...
        # Mock the entire 'utils' module before loading the checkpoint
        import mockmt3
        sys.modules['utils'] = mockmt3.utils
        ...
        # Remove the mocking to avoid side effects
        ...
        model = YourMT3(
            ...
            write_output_dir=dir_info["lightning_dir"]
            if args.write_model_output or args.test_octave_shift else None).to(device)  # modified for challenge
    ...
    precision = cfg.amt_model_precision  # @param ["32", "bf16-mixed", "16"]
    project = '2024'
    if model_name == "YMT3+":
        ...
        args = [
            checkpoint, '-p', project, '-enc', 'perceiver-tf', '-ac', 'spec', '-hop', '300', '-atc', '1', '-pr',
            precision
        ]
    elif model_name == "YPTF+Multi (PS)":
        checkpoint = "mc13_256_all_cross_v6_xk5_amp0811_edr005_attend_c_full_plus_2psn_nl26_sb_b26r_800k@model.ckpt"
        args = [
            checkpoint, '-p', project, '-tk', 'mc13_full_plus_256', '-dec', 'multi-t5', '-nl', '26', '-enc',
            'perceiver-tf', '-ac', 'spec', '-hop', '300', '-atc', '1', '-pr', precision
        ]
    elif model_name == "YPTF.MoE+Multi (noPS)":
        checkpoint = "mc13_256_g4_all_v7_mt3f_sqr_rms_moe_wf4_n8k2_silu_rope_rp_b36_nops@last.ckpt"
        args = [
            checkpoint, '-p', project, '-tk', 'mc13_full_plus_256', '-dec', 'multi-t5', '-nl', '26', '-enc',
            'perceiver-tf', '-sqr', '1', '-ff', 'moe', '-wf', '4', '-nmoe', '8', '-kmoe', '2', '-act', 'silu', '-epe',
            'rope', '-rp', '1', '-ac', 'spec', '-hop', '300', '-atc', '1', '-pr', precision
        ]
    ...
    elif model_name == "ptfs_gmlp_wf_e12_d8_500k":
        checkpoint = "ptfs_gmlp_wf_e12_d8_500k@epoch=466-step=500000.ckpt"
        project = "2024"
        args = [
            checkpoint, '-p', project, '-tk', 'mt3_full_plus_vel', '-enc', 'perceiver-tf', '-sqr', '1', '-wf', '12',
            '-ff', 'gmlp', '-dwf', '8', '-act', 'silu', '-ac', 'spec', '-hop', '300', '-atc', '1', '-ln', 'rms_norm',
            '-pr', precision
        ]
    ...
    def amt_func(model, device, audio_file, output_midi_file):
        t = Timer()

        # Converting Audio
        t.start()
        audio, sr = torchaudio.load(uri=audio_file)
        audio = torch.mean(audio, dim=0).unsqueeze(0).cpu()
        ...
        audio_segments = torch.from_numpy(audio_segments.astype('float32')).to(device).unsqueeze(
            1)  # (n_seg, 1, seg_sz)
        t.stop()
        ...
        # Inference
        t.start()
        pred_token_arr, _ = model.inference_file(bsz=cfg.amt_model_bsz, audio_segments=audio_segments)
        t.stop()
        ...
        # Write MIDI
        if '_vel' in model.task_manager.base_codec:  # check if using velocity
            input_binary_velocity = False
        else:
            input_binary_velocity = True

        print(model.midi_output_inverse_vocab)
        write_model_output_as_midi(pred_notes,
                                   cfg.amt_output_midi_dir,
                                   os.path.basename(audio_file)[:-4],
                                   model.midi_output_inverse_vocab,
                                   input_binary_velocity=input_binary_velocity)
        t.stop()
        t.print_elapsed_time("post processing")
        midifile = os.path.join(cfg.amt_output_midi_dir, 'model_output', os.path.basename(audio_file)[:-4] + '.mid')
        assert os.path.exists(midifile)

        # Move the MIDI file to the output_midi_file
        os.makedirs(os.path.dirname(output_midi_file), exist_ok=True)
        shutil.move(midifile, output_midi_file)
        return

    return model, amt_func


def get_amt_model(device, cfg, model_name='YPTF+Single (noPS)', logger=None) -> torch.nn.Module:

    model, amt_func = load_model(device, cfg, model_name)
    # logger.log_and_print("Model loaded successfully") if logger else print("Model loaded successfully")
    return model, amt_func


def download_weights(local_path, s3_uri, recursive=True):
    """Download a file or directory from S3 if it does not already exist locally."""
    if not os.path.exists(local_path):
        os.makedirs(os.path.dirname(local_path), exist_ok=True)
        print(f"📥 {local_path} not found. Downloading recursively from {s3_uri} ...")
        subprocess.run([
            "aws", "s3", "cp", s3_uri + '/checkpoints',
            os.path.dirname(local_path), "--no-sign-request", "--recursive"
        ],
                       check=True)
    else:
